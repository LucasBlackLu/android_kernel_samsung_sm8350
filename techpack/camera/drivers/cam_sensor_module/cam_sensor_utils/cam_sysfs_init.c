...
#include "cam_ois_rumba_s4.h"
	...
	defined(CONFIG_SAMSUNG_OIS_MCU_STM32)
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	cam_sensor_ssm_i2c_read(0x000A, &read_data, CAMERA_SENSOR_I2C_TYPE_WORD,
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	cam_sensor_ssm_i2c_read(0x9C6A, &read_data, CAMERA_SENSOR_I2C_TYPE_WORD,
				CAMERA_SENSOR_I2C_TYPE_BYTE);
					   ...
					   char *buf)
{
	...
	cam_sensor_ssm_i2c_read(0x9C6B, &read_data, CAMERA_SENSOR_I2C_TYPE_WORD,
				CAMERA_SENSOR_I2C_TYPE_BYTE);
...
static ssize_t rear_type_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	int rc = 0;
	...
	if (rear_fw_ver[4] == 'S')
		...
		rc = scnprintf(buf, PAGE_SIZE, "%s", cam_type_slsi);
#else
...
#if defined(CONFIG_SEC_P3Q_PROJECT) || defined(CONFIG_SEC_O3Q_PROJECT) ||      \
	defined(CONFIG_SEC_R9Q_PROJECT)
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(rear3_fw_factory_ver, sizeof(rear3_fw_factory_ver), "%s",
		  buf);
...
char rear_paf_cal_data_far[PAF_2PD_CAL_INFO_SIZE] = {
	0,
};
char rear_paf_cal_data_mid[PAF_2PD_CAL_INFO_SIZE] = {
	0,
};
...
char rear_f2_paf_cal_data_far[PAF_2PD_CAL_INFO_SIZE] = {
	0,
};
char rear_f2_paf_cal_data_mid[PAF_2PD_CAL_INFO_SIZE] = {
	0,
};
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front_cam_fw_full_ver, sizeof(front_cam_fw_full_ver), "%s",
		  buf);
	return size;
}
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front_cam_fw_user_ver, sizeof(front_cam_fw_user_ver), "%s",
		  buf);
...
static ssize_t
front_camera_firmware_factory_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front_cam_fw_factory_ver, sizeof(front_cam_fw_factory_ver),
		  "%s", buf);
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front2_cam_fw_full_ver, sizeof(front2_cam_fw_full_ver), "%s",
		  buf);
	return size;
}
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front2_cam_fw_user_ver, sizeof(front2_cam_fw_user_ver), "%s",
		  buf);
...
#endif
static ssize_t
front2_camera_firmware_factory_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	int rc = 0;
...
static ssize_t
front2_camera_firmware_factory_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t size)
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front2_cam_fw_factory_ver, sizeof(front2_cam_fw_factory_ver),
		  "%s", buf);
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front3_cam_fw_full_ver, sizeof(front3_cam_fw_full_ver), "%s",
		  buf);
	return size;
}
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front3_cam_fw_user_ver, sizeof(front3_cam_fw_user_ver), "%s",
		  buf);
...
#endif
static ssize_t
front3_camera_firmware_factory_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	int rc = 0;
...
static ssize_t
front3_camera_firmware_factory_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t size)
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(front3_cam_fw_factory_ver, sizeof(front3_cam_fw_factory_ver),
		  "%s", buf);
					   ...
					   char *buf)
{
	pr_debug("[FW_DBG] front2_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 front2_module_id[0], front2_module_id[1], front2_module_id[2],
		 front2_module_id[3], front2_module_id[4], front2_module_id[5],
		 front2_module_id[6], front2_module_id[7], front2_module_id[8],
		 front2_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       front2_module_id[0], front2_module_id[1],
		       front2_module_id[2], front2_module_id[3],
		       front2_module_id[4], front2_module_id[5],
		       front2_module_id[6], front2_module_id[7],
					   ...
					   char *buf)
{
	pr_debug("[FW_DBG] front3_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 front3_module_id[0], front3_module_id[1], front3_module_id[2],
		 front3_module_id[3], front3_module_id[4], front3_module_id[5],
		 front3_module_id[6], front3_module_id[7], front3_module_id[8],
		 front3_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       front3_module_id[0], front3_module_id[1],
		       front3_module_id[2], front3_module_id[3],
		       front3_module_id[4], front3_module_id[5],
		       front3_module_id[6], front3_module_id[7],
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(supported_camera_ids, sizeof(supported_camera_ids), "%s",
		  buf);
				  ...
				  struct device_attribute *attr, char *buf)
{
	pr_debug("[FW_DBG] rear_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 rear_module_id[0], rear_module_id[1], rear_module_id[2],
		 rear_module_id[3], rear_module_id[4], rear_module_id[5],
		 rear_module_id[6], rear_module_id[7], rear_module_id[8],
		 rear_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       rear_module_id[0], rear_module_id[1], rear_module_id[2],
		       rear_module_id[3], rear_module_id[4], rear_module_id[5],
					  ...
					  char *buf)
{
	pr_debug("[FW_DBG] front_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 front_module_id[0], front_module_id[1], front_module_id[2],
		 front_module_id[3], front_module_id[4], front_module_id[5],
		 front_module_id[6], front_module_id[7], front_module_id[8],
		 front_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       front_module_id[0], front_module_id[1],
		       front_module_id[2], front_module_id[3],
		       front_module_id[4], front_module_id[5],
		       front_module_id[6], front_module_id[7],
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	pr_debug(
		"[FW_DBG] rear3 dual tilt x = %d, y = %d, z = %d, sx = %d, sy = %d, range = %d, max_err = %d, avg_err = %d, dll_ver = %d, project_cal_type=%s\n",
		rear3_dual.x, rear3_dual.y, rear3_dual.z, rear3_dual.sx,
		rear3_dual.sy, rear3_dual.range, rear3_dual.max_err,
		rear3_dual.avg_err, rear3_dual.dll_ver,
		rear3_dual.project_cal_type);
				   ...
				   struct device_attribute *attr, char *buf)
{
	pr_debug("[FW_DBG] rear3_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 rear3_module_id[0], rear3_module_id[1], rear3_module_id[2],
		 rear3_module_id[3], rear3_module_id[4], rear3_module_id[5],
		 rear3_module_id[6], rear3_module_id[7], rear3_module_id[8],
		 rear3_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       rear3_module_id[0], rear3_module_id[1],
		       rear3_module_id[2], rear3_module_id[3],
		       rear3_module_id[4], rear3_module_id[5],
		       rear3_module_id[6], rear3_module_id[7],
		       rear3_module_id[8], rear3_module_id[9]);
}
#endif
				   ...
				   struct device_attribute *attr, char *buf)
{
	pr_debug("[FW_DBG] rear2_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 rear2_module_id[0], rear2_module_id[1], rear2_module_id[2],
		 rear2_module_id[3], rear2_module_id[4], rear2_module_id[5],
		 rear2_module_id[6], rear2_module_id[7], rear2_module_id[8],
		 rear2_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       rear2_module_id[0], rear2_module_id[1],
		       rear2_module_id[2], rear2_module_id[3],
		       rear2_module_id[4], rear2_module_id[5],
		       rear2_module_id[6], rear2_module_id[7],
		       rear2_module_id[8], rear2_module_id[9]);
}
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(rear2_fw_factory_ver, sizeof(rear2_fw_factory_ver), "%s",
		  buf);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	pr_debug(
		"[FW_DBG] rear2 dual tilt x = %d, y = %d, z = %d, sx = %d, sy = %d, range = %d, max_err = %d, avg_err = %d, dll_ver = %d, project_cal_type=%s\n",
		rear2_dual.x, rear2_dual.y, rear2_dual.z, rear2_dual.sx,
		rear2_dual.sy, rear2_dual.range, rear2_dual.max_err,
		rear2_dual.avg_err, rear2_dual.dll_ver,
		rear2_dual.project_cal_type);
...
{
	int rc = 0;
#if defined(CONFIG_SEC_Q2Q_PROJECT) || defined(CONFIG_SEC_B2Q_PROJECT) ||      \
	defined(CONFIG_SEC_V2Q_PROJECT)
	char cam_type[] = "SONY_IMX258\n";
#elif defined(CONFIG_SEC_R9Q_PROJECT)
	char cam_type_sony[] = "SONY_IMX258\n";
	char cam_type_hynix[] = "HYNIX_HI1336C\n";
#elif defined(CONFIG_SEC_O1Q_PROJECT) || defined(CONFIG_SEC_T2Q_PROJECT) ||    \
	defined(CONFIG_SEC_P3Q_PROJECT)
	char cam_type[] = "SONY_IMX563\n";
#elif defined(CONFIG_SEC_M44X_PROJECT)
...
{
	pr_debug("[FW_DBG] buf : %s\n", buf);
	scnprintf(rear4_fw_factory_ver, sizeof(rear4_fw_factory_ver), "%s",
		  buf);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	pr_debug(
		"[FW_DBG] rear4 dual tilt x = %d, y = %d, z = %d, sx = %d, sy = %d, range = %d, max_err = %d, avg_err = %d, dll_ver = %d, project_cal_type=%s\n",
		rear4_dual.x, rear4_dual.y, rear4_dual.z, rear4_dual.sx,
		rear4_dual.sy, rear4_dual.range, rear4_dual.max_err,
		rear4_dual.avg_err, rear4_dual.dll_ver,
		rear4_dual.project_cal_type);
				   ...
				   struct device_attribute *attr, char *buf)
{
	pr_debug("[FW_DBG] rear4_module_id : %c%c%c%c%c%02X%02X%02X%02X%02X\n",
		 rear4_module_id[0], rear4_module_id[1], rear4_module_id[2],
		 rear4_module_id[3], rear4_module_id[4], rear4_module_id[5],
		 rear4_module_id[6], rear4_module_id[7], rear4_module_id[8],
		 rear4_module_id[9]);
	return sprintf(buf, "%c%c%c%c%c%02X%02X%02X%02X%02X\n",
		       rear4_module_id[0], rear4_module_id[1],
		       rear4_module_id[2], rear4_module_id[3],
		       rear4_module_id[4], rear4_module_id[5],
		       rear4_module_id[6], rear4_module_id[7],
		       rear4_module_id[8], rear4_module_id[9]);
}
...
#endif
#if defined(CONFIG_SAMSUNG_OIS_MCU_STM32) ||                                   \
	defined(CONFIG_SAMSUNG_OIS_RUMBA_S4)
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < CUR_MODULE_NUM; i++)
		module_mask |= (1 << i);
	cam_ois_sine_wavecheck(g_o_ctrl, ois_autotest_threshold, buf,
			       module_mask);
			       ...
			       size_t size)
{
	...
	case '0':
		if (ois_power == 0) {
			pr_info("%s: [WARNING] ois is off, skip power down",
				__func__);
			goto error;
		}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	result = cam_ois_gyro_sensor_calibration(g_o_ctrl, &raw_data_x,
						 &raw_data_y, &raw_data_z);
#else
	result = cam_ois_gyro_sensor_calibration(g_o_ctrl, &raw_data_x,
						 &raw_data_y);
#endif
	...
	if (raw_data_x < 0 && raw_data_y < 0) {
		...
		return scnprintf(buf, PAGE_SIZE, "%d,-%ld.%03ld,-%ld.%03ld\n",
				 ...
				 abs(raw_data_x % 1000), abs(raw_data_y / 1000),
				 abs(raw_data_y % 1000));
#endif
		...
		return scnprintf(buf, PAGE_SIZE, "%d,-%ld.%03ld,%ld.%03ld\n",
				 result, abs(raw_data_x / 1000),
				 abs(raw_data_x % 1000), raw_data_y / 1000,
				 raw_data_y % 1000);
#endif
...
#else
		return scnprintf(buf, PAGE_SIZE, "%d,%ld.%03ld,-%ld.%03ld\n",
				 result, raw_data_x / 1000, raw_data_x % 1000,
				 abs(raw_data_y / 1000),
				 abs(raw_data_y % 1000));
#endif
	} else {
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	result = cam_ois_gyro_sensor_noise_check(g_o_ctrl, &stdev_data_x,
						 &stdev_data_y);

	if (stdev_data_x < 0 && stdev_data_y < 0) {
		return scnprintf(buf, PAGE_SIZE, "%d,-%ld.%03ld,-%ld.%03ld\n",
				 result, abs(stdev_data_x / 1000),
				 abs(stdev_data_x % 1000),
				 abs(stdev_data_y / 1000),
				 abs(stdev_data_y % 1000));
	} else if (stdev_data_x < 0) {
		return scnprintf(buf, PAGE_SIZE, "%d,-%ld.%03ld,%ld.%03ld\n",
				 result, abs(stdev_data_x / 1000),
				 abs(stdev_data_x % 1000), stdev_data_y / 1000,
				 stdev_data_y % 1000);
	} else if (stdev_data_y < 0) {
		return scnprintf(buf, PAGE_SIZE, "%d,%ld.%03ld,-%ld.%03ld\n",
				 result, stdev_data_x / 1000,
				 stdev_data_x % 1000, abs(stdev_data_y / 1000),
				 abs(stdev_data_y % 1000));
	} else {
		return scnprintf(buf, PAGE_SIZE, "%d,%ld.%03ld,%ld.%03ld\n",
				 result, stdev_data_x / 1000,
				 stdev_data_x % 1000, stdev_data_y / 1000,
				 stdev_data_y % 1000);
	}
}
				  ...
				  struct device_attribute *attr, char *buf)
{
...
#if defined(CONFIG_SAMSUNG_OIS_Z_AXIS_CAL)
	long raw_data_z = 0;
	result = cam_ois_offset_test(g_o_ctrl, &raw_data_x, &raw_data_y,
				     &raw_data_z, 1);
#else
...
#if defined(CONFIG_SAMSUNG_OIS_Z_AXIS_CAL)
	sprintf(buf,
		"Result : %d, result x = %ld.%03ld, result y = %ld.%03ld, result z = %ld.%03ld\n",
		result_total, raw_data_x / 1000,
		(long int)abs(raw_data_x % 1000), raw_data_y / 1000,
		(long int)abs(raw_data_y % 1000), raw_data_z / 1000,
		(long int)abs(raw_data_z % 1000));
#else
	sprintf(buf,
		"Result : %d, result x = %ld.%03ld, result y = %ld.%03ld\n",
		result_total, raw_data_x / 1000,
		(long int)abs(raw_data_x % 1000), raw_data_y / 1000,
		(long int)abs(raw_data_y % 1000));
#endif
	pr_info("%s", buf);
	...
	if (raw_data_x < 0 && raw_data_y < 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d,-%ld.%03ld,-%ld.%03ld\n",
			       result_total, (long int)abs(raw_data_x / 1000),
			       (long int)abs(raw_data_x % 1000),
			       (long int)abs(raw_data_y / 1000),
			       (long int)abs(raw_data_y % 1000));
#endif
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d,%ld.%03ld,%ld.%03ld\n",
			       result_total, raw_data_x / 1000,
			       raw_data_x % 1000, raw_data_y / 1000,
			       raw_data_y % 1000);
#endif
	}
...
static ssize_t gyro_rawdata_test_store(struct device *dev,
				       ...
				       const char *buf, size_t size)
{
	uint8_t raw_data[MAX_EFS_DATA_LENGTH] = {
		0,
	};
	long raw_data_x = 0, raw_data_y = 0;
#if defined(CONFIG_SAMSUNG_OIS_Z_AXIS_CAL)
	...
	if (ois_power) {
		if (size > MAX_EFS_DATA_LENGTH || size == 0) {
			pr_err("%s count is abnormal, count = %d", __func__,
			       size);
			return 0;
		}
...
#if defined(CONFIG_SAMSUNG_OIS_Z_AXIS_CAL)
		cam_ois_parsing_raw_data(g_o_ctrl, raw_data, efs_size,
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s: raw data x = %ld.%03ld, raw data y = %ld.%03ld, raw data z = %ld.%03ld\n",
		__func__, raw_data_x / 1000, raw_data_x % 1000,
		raw_data_y / 1000, raw_data_y % 1000, raw_data_z / 1000,
		raw_data_z % 1000);
#else
...
static ssize_t ois_exif_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	int rc = 0;
...
static ssize_t ois_exif_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t size)
{
	pr_info("[FW_DBG] buf: %s\n", buf);
				  ...
				  const char *buf, size_t size)
{
	...
	if (g_o_ctrl->is_power_up == false) {
		pr_err("%s: Fail, power down state", __func__);
		return -1;
	}
	...
	rc |= cam_ois_i2c_write(
		g_o_ctrl, 0x00BE, 0x03, CAMERA_SENSOR_I2C_TYPE_WORD,
		CAMERA_SENSOR_I2C_TYPE_BYTE); /* select module */
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (ois_gain_rear_result == 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d", ois_gain_rear_result);
	}
	if (rc)
		return rc;
	return 0;
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (ois_gain_rear3_result == 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d", ois_gain_rear3_result);
	}
	if (rc)
		return rc;
	return 0;
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (ois_gain_rear4_result == 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d", ois_gain_rear4_result);
	}
	if (rc)
		return rc;
	return 0;
					       ...
					       char *buf)
{
	...
	if (ois_sr_rear_result == 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d,%u.%02u,%u.%02u",
			       ois_sr_rear_result, (xsr / 100), (xsr % 100),
			       (ysr / 100), (ysr % 100));
	} else {
		rc = scnprintf(buf, PAGE_SIZE, "%d", ois_sr_rear_result);
	}
						...
						char *buf)
{
	...
	if (ois_sr_rear3_result == 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d,%u.%02u,%u.%02u",
			       ois_sr_rear3_result, (xsr / 100), (xsr % 100),
			       (ysr / 100), (ysr % 100));
	} else {
		rc = scnprintf(buf, PAGE_SIZE, "%d", ois_sr_rear3_result);
	}
	if (rc)
		return rc;
	return 0;
						...
						char *buf)
{
	...
	if (ois_sr_rear4_result == 0) {
		...
		rc = scnprintf(buf, PAGE_SIZE, "%d,%u.%02u,%u.%02u",
			       ois_sr_rear4_result, (xsr / 100), (xsr % 100),
			       (ysr / 100), (ysr % 100));
	} else {
		rc = scnprintf(buf, PAGE_SIZE, "%d", ois_sr_rear4_result);
	}
	if (rc)
		return rc;
	return 0;
					 ...
					 char *buf)
{
	...
	rc = scnprintf(buf, PAGE_SIZE, "%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
		       (rc < 0 ? 0 : 1), result[0], result[1], result[2],
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	uint16_t result[HALL_CAL_COUNT] = {
		0,
	};
					 ...
					 const char *buf, size_t size)
{
	...
	case '0':
		if (actuator_power == 0) {
			pr_info("%s: [WARNING] actuator is off, skip power down",
				__func__);
			goto error;
		}
		for (i = 0; i < cnt; i++) {
...
#endif
				{
					mutex_lock(&(g_a_ctrls[index]
							     ->actuator_mutex));
					rc |= cam_actuator_power_down(
						g_a_ctrls[index]);
					mutex_unlock(
						&(g_a_ctrls[index]
							  ->actuator_mutex));
				}
				pr_info("%s: actuator %u power down", __func__,
					index);
			}
		}
		if (rc < 0) {
	...
	defined(CONFIG_SAMSUNG_OIS_RUMBA_S4)
		...
		for (i = 0; i < cnt; i++) {
			...
			if (g_a_ctrls[index] != NULL) {
				...
				cam_actuator_default_init_setting(
					g_a_ctrls[index]);
				pr_info("%s: actuator %u power up", __func__,
					index);
			}
		}
		break;
					 ...
					 char *buf)
{
	...
	if (ec_param != NULL) {
		...
		case HW_PARAMS_MI_VALID:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIR_ID\":\"%c%c%c%c%cXX%02X%02X%02X\",\"I2CR_AF\":\"%d\",\"I2CR_COM\":\"%d\",\"I2CR_OIS\":\"%d\","
				"\"I2CR_SEN\":\"%d\",\"MIPIR_COM\":\"%d\",\"MIPIR_SEN\":\"%d\"\n",
				rear_module_id[0], rear_module_id[1],
				rear_module_id[2], rear_module_id[3],
				rear_module_id[4], rear_module_id[7],
				rear_module_id[8], rear_module_id[9],
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
				ec_param->i2c_sensor_err_cnt,
				ec_param->mipi_comp_err_cnt,
		...
		case HW_PARAMS_MIR_ERR_1:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIR_ID\":\"MIR_ERR\",\"I2CR_AF\":\"%d\",\"I2CR_COM\":\"%d\",\"I2CR_OIS\":\"%d\","
				"\"I2CR_SEN\":\"%d\",\"MIPIR_COM\":\"%d\",\"MIPIR_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		default:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIR_ID\":\"MI_NO\",\"I2CR_AF\":\"%d\",\"I2CR_COM\":\"%d\",\"I2CR_OIS\":\"%d\","
				"\"I2CR_SEN\":\"%d\",\"MIPIR_COM\":\"%d\",\"MIPIR_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
				...
				struct device_attribute *attr, char *buf)
{
	...
	pr_debug(
		"[FW_DBG] front2 dual tilt x = %d, y = %d, z = %d, sx = %d, sy = %d, range = %d, max_err = %d, avg_err = %d, dll_ver = %d\n",
		front2_dual.x, front2_dual.y, front2_dual.z, front2_dual.sx,
		front2_dual.sy, front2_dual.range, front2_dual.max_err,
		front2_dual.avg_err, front2_dual.dll_ver);

	rc = scnprintf(buf, PAGE_SIZE, "1 %d %d %d %d %d %d %d %d %d\n",
		       front2_dual.x, front2_dual.y, front2_dual.z,
		       front2_dual.sx, front2_dual.sy, front2_dual.range,
		       front2_dual.max_err, front2_dual.avg_err,
		       front2_dual.dll_ver);
	if (rc)
		return rc;
	return 0;
					  ...
					  char *buf)
{
	...
	if (ec_param != NULL) {
		...
		case HW_PARAMS_MI_VALID:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIF_ID\":\"%c%c%c%c%cXX%02X%02X%02X\",\"I2CF_AF\":\"%d\",\"I2CF_COM\":\"%d\",\"I2CF_OIS\":\"%d\","
				"\"I2CF_SEN\":\"%d\",\"MIPIF_COM\":\"%d\",\"MIPIF_SEN\":\"%d\"\n",
				front_module_id[0], front_module_id[1],
				front_module_id[2], front_module_id[3],
				front_module_id[4], front_module_id[7],
				front_module_id[8], front_module_id[9],
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		case HW_PARAMS_MIR_ERR_1:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIF_ID\":\"MIR_ERR\",\"I2CF_AF\":\"%d\",\"I2CF_COM\":\"%d\",\"I2CF_OIS\":\"%d\","
				"\"I2CF_SEN\":\"%d\",\"MIPIF_COM\":\"%d\",\"MIPIF_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		default:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIF_ID\":\"MI_NO\",\"I2CF_AF\":\"%d\",\"I2CF_COM\":\"%d\",\"I2CF_OIS\":\"%d\","
				"\"I2CF_SEN\":\"%d\",\"MIPIF_COM\":\"%d\",\"MIPIF_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
					  ...
					  char *buf)
{
	...
	if (ec_param != NULL) {
		...
		case HW_PARAMS_MI_VALID:
			rc = sprintf(
				buf,
				"\"CAMIR2_ID\":\"%c%c%c%c%cXX%02X%02X%02X\",\"I2CR2_AF\":\"%d\",\"I2CR2_COM\":\"%d\",\"I2CR2_OIS\":\"%d\","
				"\"I2CR2_SEN\":\"%d\",\"MIPIR2_COM\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
				rear2_module_id[0], rear2_module_id[1],
				rear2_module_id[2], rear2_module_id[3],
				rear2_module_id[4], rear2_module_id[7],
				rear2_module_id[8], rear2_module_id[9],
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		case HW_PARAMS_MIR_ERR_1:
			rc = sprintf(
				buf,
				"\"CAMIR2_ID\":\"MIR_ERR\",\"I2CR2_AF\":\"%d\",\"I2CR2_COM\":\"%d\",\"I2CR2_OIS\":\"%d\","
				"\"I2CR2_SEN\":\"%d\",\"MIPIR2_COM\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		default:
			rc = sprintf(
				buf,
				"\"CAMIR2_ID\":\"MI_NO\",\"I2CR2_AF\":\"%d\",\"I2CR2_COM\":\"%d\",\"I2CR2_OIS\":\"%d\","
				"\"I2CR2_SEN\":\"%d\",\"MIPIR2_COM\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
					  ...
					  char *buf)
{
	...
	if (ec_param != NULL) {
		...
		case HW_PARAMS_MI_VALID:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIR3_ID\":\"%c%c%c%c%cXX%02X%02X%02X\",\"I2CR3_AF\":\"%d\",\"I2CR3_COM\":\"%d\",\"I2CR3_OIS\":\"%d\","
				"\"I2CR3_SEN\":\"%d\",\"MIPIR3_COM\":\"%d\",\"MIPIR3_SEN\":\"%d\"\n",
				rear3_module_id[0], rear3_module_id[1],
				rear3_module_id[2], rear3_module_id[3],
				rear3_module_id[4], rear3_module_id[7],
				rear3_module_id[8], rear3_module_id[9],
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		case HW_PARAMS_MIR_ERR_1:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIR3_ID\":\"MIR_ERR\",\"I2CR3_AF\":\"%d\",\"I2CR3_COM\":\"%d\",\"I2CR3_OIS\":\"%d\","
				"\"I2CR3_SEN\":\"%d\",\"MIPIR3_COM\":\"%d\",\"MIPIR3_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		default:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIR3_ID\":\"MI_NO\",\"I2CR3_AF\":\"%d\",\"I2CR3_COM\":\"%d\",\"I2CR3_OIS\":\"%d\","
				"\"I2CR3_SEN\":\"%d\",\"MIPIR3_COM\":\"%d\",\"MIPIR3_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
					   ...
					   char *buf)
{
	...
	if (ec_param != NULL) {
		...
		case HW_PARAMS_MI_VALID:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIF2_ID\":\"%c%c%c%c%cXX%02X%02X%02X\",\"I2CF2_AF\":\"%d\",\"I2CF2_COM\":\"%d\",\"I2CF2_OIS\":\"%d\","
				"\"I2CF2_SEN\":\"%d\",\"MIPIF2_COM\":\"%d\",\"MIPIF2_SEN\":\"%d\"\n",
				front2_module_id[0], front2_module_id[1],
				front2_module_id[2], front2_module_id[3],
				front2_module_id[4], front2_module_id[7],
				front2_module_id[8], front2_module_id[9],
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		case HW_PARAMS_MIR_ERR_1:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIF2_ID\":\"MIR_ERR\",\"I2CF2_AF\":\"%d\",\"I2CF2_COM\":\"%d\",\"I2CF2_OIS\":\"%d\","
				"\"I2CF2_SEN\":\"%d\",\"MIPIF2_COM\":\"%d\",\"MIPIF2_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
		...
		default:
			rc = scnprintf(
				buf, PAGE_SIZE,
				"\"CAMIF2_ID\":\"MI_NO\",\"I2CF2_AF\":\"%d\",\"I2CF2_COM\":\"%d\",\"I2CF2_OIS\":\"%d\","
				"\"I2CF2_SEN\":\"%d\",\"MIPIF2_COM\":\"%d\",\"MIPIF2_SEN\":\"%d\"\n",
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
	...
	if (ec_param != NULL) {
		...
		case HW_PARAMS_MI_VALID:
			rc = scnprintf(
				...
				front3_module_id[0], front3_module_id[1],
				front3_module_id[2], front3_module_id[3],
				front3_module_id[4], front3_module_id[7],
				front3_module_id[8], front3_module_id[9],
				ec_param->i2c_af_err_cnt,
				ec_param->i2c_comp_err_cnt,
				ec_param->i2c_ois_err_cnt,
...
ssize_t rear_flash_store(struct device *dev, struct device_attribute *attr,
...
ssize_t rear_flash_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
#if IS_REACHABLE(CONFIG_LEDS_S2MPB02)
...
static DEVICE_ATTR(rear_camfw, S_IRUGO | S_IWUSR | S_IWGRP, rear_firmware_show,
		   rear_firmware_store);
...
static DEVICE_ATTR(rear_paf_offset_far, S_IRUGO, rear_paf_offset_far_show,
		   NULL);
static DEVICE_ATTR(rear_paf_offset_mid, S_IRUGO, rear_paf_offset_mid_show,
		   NULL);
static DEVICE_ATTR(rear_paf_cal_check, S_IRUGO, rear_paf_cal_check_show, NULL);
static DEVICE_ATTR(rear_f2_paf_offset_far, S_IRUGO, rear_f2_paf_offset_far_show,
		   NULL);
static DEVICE_ATTR(rear_f2_paf_offset_mid, S_IRUGO, rear_f2_paf_offset_mid_show,
		   NULL);
static DEVICE_ATTR(rear_f2_paf_cal_check, S_IRUGO, rear_f2_paf_cal_check_show,
		   NULL);
#if 0 //EARLY_RETENTION
...
static DEVICE_ATTR(front_camfw_full, S_IRUGO | S_IWUSR | S_IWGRP,
		   front_camera_firmware_full_show,
		   front_camera_firmware_full_store);
static DEVICE_ATTR(front_checkfw_user, S_IRUGO | S_IWUSR | S_IWGRP,
		   front_camera_firmware_user_show,
		   front_camera_firmware_user_store);
static DEVICE_ATTR(front_checkfw_factory, S_IRUGO | S_IWUSR | S_IWGRP,
		   front_camera_firmware_factory_show,
...
#endif
static DEVICE_ATTR(front_paf_cal_check, S_IRUGO, front_paf_cal_check_show,
		   NULL);
...
#if defined(CONFIG_CAMERA_ADAPTIVE_MIPI)
static DEVICE_ATTR(front_mipi_clock, S_IRUGO, front_camera_mipi_clock_show,
		   NULL);
#endif
#if defined(CONFIG_SAMSUNG_FRONT_DUAL)
...
static DEVICE_ATTR(front2_camfw_full, S_IRUGO | S_IWUSR | S_IWGRP,
		   front2_camera_firmware_full_show,
		   front2_camera_firmware_full_store);
static DEVICE_ATTR(front2_checkfw_user, S_IRUGO | S_IWUSR | S_IWGRP,
		   front2_camera_firmware_user_show,
		   front2_camera_firmware_user_store);
static DEVICE_ATTR(front2_checkfw_factory, S_IRUGO | S_IWUSR | S_IWGRP,
		   front2_camera_firmware_factory_show,
		   front2_camera_firmware_factory_store);
...
static DEVICE_ATTR(front3_camfw_full, S_IRUGO | S_IWUSR | S_IWGRP,
		   front3_camera_firmware_full_show,
		   front3_camera_firmware_full_store);
static DEVICE_ATTR(front3_checkfw_user, S_IRUGO | S_IWUSR | S_IWGRP,
		   front3_camera_firmware_user_show,
		   front3_camera_firmware_user_store);
static DEVICE_ATTR(front3_checkfw_factory, S_IRUGO | S_IWUSR | S_IWGRP,
		   front3_camera_firmware_factory_show,
		   front3_camera_firmware_factory_store);
...
static DEVICE_ATTR(SVC_upper_module, S_IRUGO, front3_camera_moduleid_show,
		   NULL);
#if defined(CONFIG_CAMERA_SYSFS_V2)
...
#endif
static DEVICE_ATTR(front2_moduleid, S_IRUGO, front2_camera_moduleid_show, NULL);
static DEVICE_ATTR(SVC_upper_module, S_IRUGO, front2_camera_moduleid_show,
		   NULL);
#if defined(CONFIG_CAMERA_SYSFS_V2)
static DEVICE_ATTR(front2_caminfo, S_IRUGO | S_IWUSR | S_IWGRP,
		   front2_camera_info_show, front2_camera_info_store);
...
static DEVICE_ATTR(rear3_paf_cal_check, S_IRUGO, rear3_paf_cal_check_show,
		   NULL);
#if defined(CONFIG_SEC_P3Q_PROJECT) || defined(CONFIG_SEC_O3Q_PROJECT) ||      \
	defined(CONFIG_SEC_R9Q_PROJECT)
...
static DEVICE_ATTR(rear2_paf_cal_check, S_IRUGO, rear2_paf_cal_check_show,
		   NULL);
#endif
...
static DEVICE_ATTR(rear2_moduleid, S_IRUGO, rear2_moduleid_show, NULL);
...
static DEVICE_ATTR(rear4_paf_cal_check, S_IRUGO, rear4_paf_cal_check_show,
		   NULL);
...
static DEVICE_ATTR(front2_dualcal_size, S_IRUGO, front2_dual_cal_size_show,
		   NULL);
...
static DEVICE_ATTR(rear_actuator_power, S_IWUSR | S_IWGRP, NULL,
		   rear_actuator_power_store);

#if defined(CONFIG_SAMSUNG_OIS_MCU_STM32) ||                                   \
	defined(CONFIG_SAMSUNG_OIS_RUMBA_S4)
static DEVICE_ATTR(ois_power, S_IWUSR, NULL, ois_power_store);
static DEVICE_ATTR(autotest, S_IRUGO | S_IWUSR | S_IWGRP, ois_autotest_show,
		   ois_autotest_store);
...
static DEVICE_ATTR(ois_rawdata, S_IRUGO | S_IWUSR | S_IWGRP,
		   gyro_rawdata_test_show, gyro_rawdata_test_store);
static DEVICE_ATTR(oisfw, S_IRUGO | S_IWUSR | S_IWGRP, ois_fw_full_show,
		   ois_fw_full_store);
static DEVICE_ATTR(ois_exif, S_IRUGO | S_IWUSR | S_IWGRP, ois_exif_show,
		   ois_exif_store);
...
static DEVICE_ATTR(ois_supperssion_ratio_rear, S_IRUGO,
		   ois_supperssion_ratio_rear_show, NULL);
...
static DEVICE_ATTR(ois_supperssion_ratio_rear3, S_IRUGO,
		   ois_supperssion_ratio_rear3_show, NULL);
static DEVICE_ATTR(rear3_read_cross_talk, S_IRUGO,
		   ois_rear3_read_cross_talk_show, NULL);
#endif
#if defined(CONFIG_SAMSUNG_REAR_QUADRA)
static DEVICE_ATTR(ois_gain_rear4, S_IRUGO, ois_gain_rear4_show, NULL);
static DEVICE_ATTR(ois_supperssion_ratio_rear4, S_IRUGO,
		   ois_supperssion_ratio_rear4_show, NULL);
static DEVICE_ATTR(rear4_read_cross_talk, S_IRUGO,
		   ois_rear4_read_cross_talk_show, NULL);
#endif
...
static DEVICE_ATTR(ois_ext_clk, S_IRUGO | S_IWUSR | S_IWGRP, ois_ext_clk_show,
		   ois_ext_clk_store);
#endif
...
static DEVICE_ATTR(af_position, S_IRUGO | S_IWUSR | S_IWGRP, af_position_show,
		   af_position_store);
...
static DEVICE_ATTR(fallback, S_IRUGO | S_IWUSR | S_IWGRP, dual_fallback_show,
		   dual_fallback_store);
#endif
...
#if defined(CONFIG_CAMERA_SSM_I2C_ENV)
	&dev_attr_ssm_frame_id, &dev_attr_ssm_gmc_state,
...
#if defined(CONFIG_CAMERA_SSM_I2C_ENV)
	cam_dev_ssm = device_create(camera_class, NULL, 0, NULL, "ssm");
...
#if defined(CONFIG_SAMSUNG_REAR_TRIPLE)
	cam_dev_af = device_create(camera_class, NULL, 1, NULL, "af");
	ret |= cam_device_create_files(cam_dev_af, af_attrs);

	cam_dev_dual = device_create(camera_class, NULL, 1, NULL, "dual");
	ret |= cam_device_create_files(cam_dev_dual, dual_attrs);
#endif

#if defined(CONFIG_SAMSUNG_OIS_MCU_STM32) ||                                   \
	defined(CONFIG_SAMSUNG_OIS_RUMBA_S4)
	cam_dev_ois = device_create(camera_class, NULL, 0, NULL, "ois");
	ret |= cam_device_create_files(cam_dev_ois, ois_attrs);
#endif

#if defined(CONFIG_CAMERA_ADAPTIVE_MIPI)
	cam_dev_test = device_create(camera_class, NULL, 0, NULL, "test");
