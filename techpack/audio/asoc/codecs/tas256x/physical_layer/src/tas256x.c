...
int tas256x_update_default_params(struct tas256x_priv *p_tas256x, int ch)
{
	...
	if (p_tas256x->devs[ch-1]->device_id == DEVICE_TAS2562) {
		...
		p_tas256x->devs[ch-1]->receiver_enable = p_tas256x->rcv_only;
	...
	int ch)
{
	...
	/* Bypass Algo in receiver mode */
	p_tas256x->algo_bypass = enable;
	...
	if (enable) {
		if (enable == 1) {
			...
			/* This register was changed for improved POP */
			n_result |=
				p_tas256x->write(p_tas256x, ch, TAS256X_CLK_SWEEP,
					0x00);

			/* DMIN was reduced to 2 for better noise levels */
			n_result |= p_tas256x->write(p_tas256x, ch, TAS256X_DMIN_VALUE,
				0x22);
			...
			/*Enable Noise Gate upon ICN detection*/
			n_result = p_tas256x->update_bits(p_tas256x, ch,
				TAS256X_ICN_SW_REG,
				TAS256X_ICN_NG_MASK,
				TAS256X_ICN_NG_ENABLE);

			/* For better idle channel noise and noise levels with input signal */
			n_result |= tas256x_enable_cm_hysterysis(p_tas256x, 1, ch);
		}
	} else {
		/*Unlock test page*/
		n_result =
