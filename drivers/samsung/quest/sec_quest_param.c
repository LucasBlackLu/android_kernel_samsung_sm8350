...
void print_result_with_item_string(uint64_t item_result, char **str_array)
{
	...
	for (item = 1; item < max_item; item++) {
		...
		results_str_size +=
			snprintf((char *)(results_str + results_str_size),
				 MAX_LEN_STR - results_str_size, " %s(%c)",
				 str_array[item], result_char);
	}
...
void quest_print_param_quest_data(void)
{
	...
	QUEST_PRINT(
		"hlos_count(%d) quefi_count(%d) suefi_count(%d) ddrscan_count(%d), num_smd_try(%d)\n",
	...
	print_result_with_item_string(param_quest_data.smd_item_result,
				      STR_ITEM);
	...
	print_result_with_item_string(param_quest_data.cal_item_result,
				      STR_ITEM);
	...
	print_result_with_item_string(param_quest_data.main_item_result,
				      STR_ITEM);
	...
	print_result_with_item_string(param_quest_data.smd_subitem_result,
				      STR_SUBITEM);
	...
	print_result_with_item_string(param_quest_data.smd_subitem_result_first,
				      STR_SUBITEM);
...
{
	if (!sec_get_param(param_index_quest_ddr_result,
			   &param_quest_ddr_result_data))
		QUEST_PRINT("%s : failed\n", __func__);
	else
		QUEST_PRINT("%s : succeeded\n", __func__);
}
...
{
	if (!sec_set_param(param_index_quest_ddr_result,
			   &param_quest_ddr_result_data))
		QUEST_PRINT("%s : failed\n", __func__);
	else
		QUEST_PRINT("%s : succeeded\n", __func__);
...
{
	if (!sec_get_param(param_index_api_gpio_test_result,
			   param_api_gpio_test_result))
		QUEST_PRINT("%s : failed\n", __func__);
	else
		QUEST_PRINT("%s : succeeded\n", __func__);
}
...
{
	if (!sec_set_param(param_index_api_gpio_test_result,
			   param_api_gpio_test_result))
		QUEST_PRINT("%s : failed\n", __func__);
	else
		QUEST_PRINT("%s : succeeded\n", __func__);
