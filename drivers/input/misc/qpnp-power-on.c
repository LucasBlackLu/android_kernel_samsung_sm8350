...
again:
	...
	switch (cfg->pon_type) {
	case PON_KPDPWR:
		pon_rt_bit = kpdwwr_bit;
		break;
	case PON_RESIN:
		pon_rt_bit = resin_bit;
		break;
	case PON_CBLPWR:
	...
	if (first) {
		...
		if (pon_rt_sts & resin_bit) {
			pon_type = PON_RESIN;
			goto again;
		} else if (pon_rt_sts &	kpdwwr_bit) {
			pon_type = PON_KPDPWR;
			goto again;
		}
	}
...
qpnp_pon_config_input(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	if (!pon->pon_input) {
		...
		if (!pon->pon_input)
			return -ENOMEM;
#if IS_ENABLED(CONFIG_SEC_B2Q_PROJECT)
		pon->pon_input->name = "qpnp_pon_flex";
		pon->pon_input->phys = "qpnp_pon_flex/input0";
#else
				       ...
				       struct device_node *node)
{
	...
	resin_bit = (is_pon_gen3(pon) ? QPNP_PON_GEN3_RESIN_N_SET : QPNP_PON_RESIN_N_SET);
...
int sec_get_s2_reset_onoff(void)
{
	int rc;
	uint val;
	struct qpnp_pon *pon = sys_reset_dev;

	rc = qpnp_pon_read(pon, QPNP_PON_KPDPWR_RESIN_S2_CNTL2(pon), &val);
	if (rc) {
		dev_err(pon->dev, "Unable to get S2 reset on/off for KPDPWR AND RESIN\n");
		return rc;
	}
...
EXPORT_SYMBOL_GPL(sec_get_s2_reset_onoff);

int sec_set_s2_reset_onoff(int val)
{
	int rc;
	struct qpnp_pon *pon = sys_reset_dev;

	/* Configure  s2 reset on/off for KPDPWR AND RESIN */
	rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_RESIN_S2_CNTL2(pon),
							   QPNP_PON_S2_CNTL_EN, val ? QPNP_PON_S2_CNTL_EN :0);
	if (rc) {
		dev_err(pon->dev, "Unable to configure S2 reset for KPDPWR AND RESIN\n");
		return rc;
	}

	return 0;
}
EXPORT_SYMBOL_GPL(sec_set_s2_reset_onoff);
#endif
