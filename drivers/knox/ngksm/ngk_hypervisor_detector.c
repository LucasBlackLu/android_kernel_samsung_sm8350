...
#define FASTUH_APP_HDM                6

unsigned long fastuh_call(u64 app_id, u64 command, u64 arg0, u64 arg1, u64 arg2, u64 arg3)
{
	return 0;
}
#endif
...
static void apply_execlude_policy(const u64 policy, const u64 mode)
{
	...
	fastuh_call(FASTUH_APP_HDM, PAD_HYP_CMD, (u64)&hyp_return, (u64)subsystem, 0, 0);
...
static void onetime_hyp_status_check(void)
{
	...
	status = 0;
	fastuh_call(FASTUH_APP_HDM, PAD_HYP_GET_STATUS, (u64)&status, 0, 0, 0);
...
void __exit ngk_hyp_detector_exit(void)
{
	...
	fastuh_call(FASTUH_APP_HDM, PAD_HYP_CMD, (u64)&result, 0, 0, 0);
	kthread_stop(hyp_detector_thread.polling_thread);
	hyp_detector_thread.is_started = false;
	hyp_detector_thread.applied_subsystem = 0;
}
