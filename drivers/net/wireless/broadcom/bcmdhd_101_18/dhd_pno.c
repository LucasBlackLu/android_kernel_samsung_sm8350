...
#ifdef GSCAN_SUPPORT
static uint64 convert_fw_rel_time_to_systime(struct timespec64 *ts,
					     uint32 fw_ts_ms)
{
	return ((uint64)(TIMESPEC64_TO_US(*ts)) -
		(uint64)(fw_ts_ms * USEC_PER_MSEC));
}
				...
				dhd_epno_results_t *res, uint32 idx)
{
	...
	if (gscan_params->epno_cfg.num_epno_ssid > 0) {
		...
		list_for_each_entry_safe (
			iter, next, &gscan_params->epno_cfg.epno_ssid_list,
			list) {
			GCC_DIAGNOSTIC_POP();
			if (i++ == idx) {
...
static int _dhd_pno_gscan_cfg(dhd_pub_t *dhd,
			      wl_pfn_gscan_cfg_t *pfncfg_gscan_param, int size)
{
	int err = BCME_OK;
	NULL_CHECK(dhd, "dhd is NULL", err);
...
static int _dhd_pno_flush_ssid(dhd_pub_t *dhd)
{
	...
	err = dhd_iovar(dhd, 0, "pfn_add", (char *)&pfn_elem, sizeof(wl_pfn_t),
			NULL, 0, TRUE);
	if (err < 0) {
...
static int _dhd_pno_enable(dhd_pub_t *dhd, int enable)
{
	...
	/* Enable/Disable PNO */
	err = dhd_iovar(dhd, 0, "pfn", (char *)&enable, sizeof(enable), NULL, 0,
			TRUE);
	if (err < 0) {
			...
			dhd_pno_mode_t mode)
{
	...
	pfn_param.flags = ((PFN_LIST_ORDER << SORT_CRITERIA_BIT) |
			   (ENABLE << IMMEDIATE_SCAN_BIT) |
			   (ENABLE << REPORT_SEPERATELY_BIT));
	if (mode == DHD_PNO_LEGACY_MODE) {
		pfn_param.repeat =
			(uchar)(pno_params->params_legacy.pno_repeat);
		/* check and set extra pno params */
		if ((pno_params->params_legacy.pno_repeat != 0) &&
		    (pno_params->params_legacy.pno_freq_expo_max != 0)) {
			pfn_param.flags |=
				htod16(ENABLE << ENABLE_ADAPTSCAN_BIT);
			pfn_param.exp = (uchar)(pno_params->params_legacy
							.pno_freq_expo_max);
		}
		/* set up pno scan fr */
		if (pno_params->params_legacy.scan_fr != 0)
			pfn_param.scan_freq =
				htod32(pno_params->params_legacy.scan_fr);
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			DHD_PNO((
				"will enable combined scan with BATCHIG SCAN MODE\n"));
			mode |= DHD_PNO_BATCH_MODE;
			combined_scan = TRUE;
		} else if (_pno_state->pno_mode & DHD_PNO_HOTLIST_MODE) {
			DHD_PNO((
				"will enable combined scan with HOTLIST SCAN MODE\n"));
			...
			DHD_PNO((
				"will enable combined scan with GSCAN SCAN MODE\n"));
	...
	if (mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		...
		if (mode == DHD_PNO_BATCH_MODE) {
			...
			if (pno_params->params_batch.bestn)
				pfn_param.bestn =
					pno_params->params_batch.bestn;
			if (pno_params->params_batch.scan_fr)
				pfn_param.scan_freq = htod32(
					pno_params->params_batch.scan_fr);
			if (pno_params->params_batch.mscan)
				pfn_param.mscan =
					pno_params->params_batch.mscan;
			...
			if (pno_params->params_hotlist.scan_fr)
				pfn_param.scan_freq = htod32(
					pno_params->params_hotlist.scan_fr);
			pfn_param.bestn = 0;
			pfn_param.repeat = 0;
			/* enable broadcast scan */
			pfn_param.flags |= (ENABLE << ENABLE_BD_SCAN_BIT);
		...
		if (combined_scan) {
			...
			if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
				...
				if (_params->params_hotlist.scan_fr)
					pfn_param.scan_freq = htod32(
						_params->params_hotlist.scan_fr);
				pfn_param.bestn = 0;
				pfn_param.repeat = 0;
			}
		}
	...
	if (mode & DHD_PNO_GSCAN_MODE) {
		...
		if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
			...
			params = &(_pno_state->pno_params_arr
					   [INDEX_OF_LEGACY_PARAMS]);

			pfn_param.scan_freq =
				gcd(pno_params->params_gscan.scan_fr,
				    params->params_legacy.scan_fr);

			if ((params->params_legacy.pno_repeat != 0) ||
			    (params->params_legacy.pno_freq_expo_max != 0)) {
				pfn_param.repeat = (uchar)(params->params_legacy
								   .pno_repeat);
				pfn_param.exp =
					(uchar)(params->params_legacy
							.pno_freq_expo_max);
			}
		}

		lost_network_timeout =
			(pno_params->params_gscan.max_ch_bucket_freq *
			 pfn_param.scan_freq *
			 pno_params->params_gscan.lost_ap_window);
		if (lost_network_timeout) {
			pfn_param.lost_network_timeout = htod32(MIN(
				lost_network_timeout, GSCAN_MIN_BSSID_TIMEOUT));
		} else {
			     ...
			     int nssid)
{
	...
	list_for_each_entry_safe (iter, next, ssid_list, list) {
		...
		if (iter->hidden)
			pfn_elem_buf[i].flags |=
				htod32(ENABLE << WL_PFN_HIDDEN_BIT);
...
#else
		pfn_elem_buf[i].flags |=
			((iter->rssi_thresh & 0xFF) << WL_PFN_RSSI_SHIFT);
		...
		DHD_PNO(("%s size = %d hidden = %d flags = %x rssi_thresh %d\n",
			 iter->SSID, iter->SSID_len, iter->hidden, iter->flags,
			 iter->rssi_thresh));
		if (++i >= nssid) {
			/* shouldn't happen */
			break;
	...
	err = dhd_iovar(dhd, 0, "pfn_add", (char *)pfn_elem_buf, mem_needed,
			NULL, 0, TRUE);
	if (err < 0) {
		DHD_ERROR(("%s : failed to execute pfn_add\n", __FUNCTION__));
	}
			       ...
			       uint16 *chan_list2, int nchan2)
{
	...
	while (i < nchan1 && j < nchan2) {
		tmp = chan_list1[i] < chan_list2[j] ? chan_list1[i++] :
						      chan_list2[j++];
				 ...
				 int *nchan, uint8 band, bool skip_dfs)
{
	...
	for (i = 0, j = 0; i < dtoh32(list->count) && i < *nchan; i++) {
		if (IS_2G_CHANNEL(dtoh32(list->element[i]))) {
			...
			bool dfs_channel =
				is_dfs(dhd, dtoh32(list->element[i]));
				   ...
				   char *buf, int nbufsize)
{
	...
	if (!params_batch->get_batch.batch_started) {
		bp += nreadsize =
			snprintf(bp, nleftsize, "scancount=%d\n",
	...
	list_for_each_entry_safe (
		...
		while (phead != NULL) {
			...
			list_for_each_entry_safe (iter, next,
				...
				GCC_DIAGNOSTIC_POP();
				t_delta = jiffies_to_msecs(jiffies -
							   iter->recorded_time);
#ifdef PNO_DEBUG
				_base_bp = bp;
				...
				/* RTT1 */
				bp += nreadsize = snprintf(
					bp, nleftsize, "distSd=%d\n",
					(iter->rtt0 == 0) ? -1 : iter->rtt1);
				nleftsize -= nreadsize;
				bp += nreadsize = snprintf(bp, nleftsize, "%s",
							   AP_END_MARKER);
				nleftsize -= nreadsize;
				list_del(&iter->list);
			...
			bp += nreadsize =
				snprintf(bp, nleftsize, "%s", SCAN_END_MARKER);
	...
	if (nchan) {
		...
		for (i = 0; i < nchan; i++) {
			if (dhd->wlc_ver_major >= DHD_PNO_CHSPEC_SUPPORT_VER) {
				pfncfg_param.channel_list[i] =
					wf_chspec_ctlchspec(channel_list[i]);
			} else {
				      ...
				      dhd_pno_mode_t mode)
{
	...
	case DHD_PNO_LEGACY_MODE: {
		...
		if (params->params_legacy.nssid > 0) {
			GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
			list_for_each_entry_safe (
				iter, next, &params->params_legacy.ssid_list,
				list) {
				GCC_DIAGNOSTIC_POP();
				list_del(&iter->list);
				MFREE(dhd->osh, iter,
	...
	case DHD_PNO_BATCH_MODE: {
		...
		_dhd_pno_clear_all_batch_results(
			dhd, &params->params_batch.get_batch.scan_results_list,
			FALSE);
		_dhd_pno_clear_all_batch_results(
			dhd,
			&params->params_batch.get_batch
		...
		INIT_LIST_HEAD(
			&params->params_batch.get_batch.scan_results_list);
		INIT_LIST_HEAD(&params->params_batch.get_batch
					.expired_scan_results_list);
		break;
	}
	case DHD_PNO_HOTLIST_MODE: {
		...
		if (params->params_hotlist.nbssid > 0) {
			GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
			list_for_each_entry_safe (
				iter, next, &params->params_hotlist.bssid_list,
				list) {
				GCC_DIAGNOSTIC_POP();
				list_del(&iter->list);
				MFREE(dhd->osh, iter,
...
int dhd_pno_stop_for_ssid(dhd_pub_t *dhd)
{
	...
	if (_pno_state->pno_mode &
	    (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		/* retrieve the batching data from firmware into host */
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			_params = &(
				_pno_state
					->pno_params_arr[INDEX_OF_BATCH_PARAMS]);
			/* restart BATCH SCAN */
			err = dhd_pno_set_for_batch(dhd,
						    &_params->params_batch);
			if (err < 0) {
				_pno_state->pno_mode &= ~DHD_PNO_BATCH_MODE;
				DHD_ERROR((
					"%s : failed to restart batch scan(err: %d)\n",
					__FUNCTION__, err));
				goto exit;
			}
		} else if (_pno_state->pno_mode & DHD_PNO_HOTLIST_MODE) {
			...
			list_for_each_entry_safe (
				iter, next, &_params->params_hotlist.bssid_list,
				list) {
				GCC_DIAGNOSTIC_POP();
				memcpy(&tmp_bssid->macaddr, &iter->macaddr,
				       ETHER_ADDR_LEN);
				...
				if (cnt < _params->params_hotlist.nbssid) {
					...
					DHD_ERROR((
						"%s: Allocated insufficient memory\n",
						__FUNCTION__));
					break;
				}
			}
			err = dhd_pno_set_for_hotlist(dhd, p_pfn_bssid,
						      &_params->params_hotlist);
			if (err < 0) {
				_pno_state->pno_mode &= ~DHD_PNO_HOTLIST_MODE;
				DHD_ERROR((
					"%s : failed to restart hotlist scan(err: %d)\n",
					__FUNCTION__, err));
				goto exit;
			}
		}
				    ...
				    int *num_ssid_added)
{
	...
	for (i = 0; i < nssid; i++) {
		...
		if (!ssid_list[i].SSID_len) {
			DHD_ERROR((
				"%d: Broadcast SSID is illegal for PNO setting\n",
				i));
			ret = BCME_ERROR;
			goto exit;
		}
		_pno_ssid = (struct dhd_pno_ssid *)MALLOCZ(
			dhd->osh, sizeof(struct dhd_pno_ssid));
		if (_pno_ssid == NULL) {
			DHD_ERROR((
				  ...
				  uint16 *channel_list, int nchan)
{
	...
	if (_pno_state->pno_mode &
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			...
			_params2 = &(_pno_state->pno_params_arr
					     [INDEX_OF_HOTLIST_PARAMS]);
			if (_params2->params_hotlist.nchan > 0 &&
			    tot_nchan > 0) {
				err = _dhd_pno_chan_merge(
					_chan_list, &tot_nchan,
	...
	if ((err = _dhd_pno_cfg(dhd, _chan_list, tot_nchan)) < 0) {
		DHD_ERROR((
			"%s : failed to set call pno_cfg (err %d) in firmware\n",
			__FUNCTION__, err));
		goto exit;
	}
			  ...
			  struct dhd_pno_batch_params *batch_params)
{
	...
	if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
		_pno_state->pno_mode |= DHD_PNO_BATCH_MODE;
		err = _dhd_pno_reinitialize_prof(dhd, _params,
						 DHD_PNO_BATCH_MODE);
		if (err < 0) {
			DHD_ERROR((
	...
	if (batch_params->band == WLC_BAND_2G ||
		...
		if (err < 0) {
			DHD_ERROR((
				"%s: failed to get valid channel list(band : %d)\n",
		...
		sort(_params->params_batch.chan_list,
		     _params->params_batch.nchan,
		     sizeof(_params->params_batch.chan_list[0]),
		     _dhd_pno_cmpfunc, NULL);
	}
#ifdef PNO_DEBUG
	{
	...
	if (_params->params_batch.nchan) {
		...
		memcpy(_chan_list, _params->params_batch.chan_list,
		       sizeof(_chan_list));
	...
	if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
		...
		if (_params2->params_legacy.nchan > 0 &&
			...
			DHD_PNO((
				"superset channel will use all channels in firmware\n"));
		}
		if ((err = _dhd_pno_add_ssid(
			     dhd, &_params2->params_legacy.ssid_list,
			     _params2->params_legacy.nssid)) < 0) {
			DHD_ERROR((
				"failed to add ssid list (err %d) in firmware\n",
				err));
			goto exit;
		}
	}
	if ((err = _dhd_pno_set(dhd, _params, DHD_PNO_BATCH_MODE)) < 0) {
		DHD_ERROR((
			"%s : failed to set call pno_set (err %d) in firmware\n",
			__FUNCTION__, err));
		goto exit;
	} else {
...
int dhd_pno_set_epno(dhd_pub_t *dhd)
{
	...
	if (gscan_params->epno_cfg.num_epno_ssid) {
		DHD_PNO(("num_epno_ssid %d\n",
			 gscan_params->epno_cfg.num_epno_ssid));
		if ((err = _dhd_pno_add_ssid(
			     dhd, &gscan_params->epno_cfg.epno_ssid_list,
			     gscan_params->epno_cfg.num_epno_ssid)) < 0) {
			DHD_ERROR((
				"failed to add ssid list (err %d) to firmware\n",
				err));
			return err;
		}
		err = dhd_set_epno_params(dhd, &gscan_params->epno_cfg.params,
					  TRUE);
		if (err < 0) {
			DHD_ERROR(("failed to set ePNO params %d\n", err));
		}
				    ...
				    uint8 flags)
{
	...
	if (flags & GSCAN_FLUSH_EPNO_CFG) {
		...
		if (epno_cfg->num_epno_ssid > 0) {
			GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
			list_for_each_entry_safe (
				iter, next, &epno_cfg->epno_ssid_list, list) {
				GCC_DIAGNOSTIC_POP();
				list_del(&iter->list);
				MFREE(dhd->osh, iter,
				      sizeof(struct dhd_pno_bssid));
	...
	if (_params->params_gscan.get_batch_flag ==
		...
		if ((_pno_state->pno_mode & DHD_PNO_GSCAN_MODE) &&
			...
			if (err == BCME_OK) {
				wait_event_interruptible_timeout(
					_pno_state->batch_get_wait,
					is_batch_retrieval_complete(
						&_params->params_gscan),
					msecs_to_jiffies(
						GSCAN_BATCH_GET_MAX_WAIT));
			}
		}
	}
			  ...
			  void *buf, bool flush)
{
	...
	case DHD_PNO_GEOFENCE_SCAN_CFG_ID: {
		...
		if ((_params->params_gscan.nbssid_hotlist + ptr->nbssid) >
		    PFN_SWC_MAX_NUM_APS) {
			DHD_ERROR((
				"Excessive number of hotlist APs programmed %d\n",
		...
		for (i = 0, bssid_ptr = ptr->bssid; i < ptr->nbssid;
		     i++, bssid_ptr++) {
			_pno_bssid = (struct dhd_pno_bssid *)MALLOCZ(
				dhd->osh, sizeof(struct dhd_pno_bssid));
			if (!_pno_bssid) {
				DHD_ERROR((
					"_pno_bssid is NULL, cannot kalloc %zd bytes",
					sizeof(struct dhd_pno_bssid)));
				err = BCME_NOMEM;
				goto exit;
			}
	...
	case DHD_PNO_SCAN_CFG_ID: {
		...
		if (ptr->nchannel_buckets <= GSCAN_MAX_CH_BUCKETS) {
			...
			for (i = 0; i < ptr->nchannel_buckets; i++) {
				...
				for (k = 0;
				     k < ptr->channel_bucket[i].num_channels;
				     k++) {
					ch_bucket[i].chan_list[k] =
						wf_mhz2channel(
							ptr->channel_bucket[i]
								.chan_list[k],
							0);
				}
				ch_bucket[i].band = 0;
...
static bool validate_gscan_params(struct dhd_pno_gscan_params *gscan_params)
{
	...
	if (!gscan_params->scan_fr || !gscan_params->nchannel_buckets) {
		DHD_ERROR((
			...
			__FUNCTION__, gscan_params->scan_fr,
			gscan_params->nchannel_buckets));
		return false;
	}
	...
	for (i = 0; i < gscan_params->nchannel_buckets; i++) {
		if (!gscan_params->channel_bucket[i].band) {
			for (k = 0;
			     k < gscan_params->channel_bucket[i].num_channels;
			     k++) {
				if (gscan_params->channel_bucket[i]
					    .chan_list[k] > CHANNEL_5G_MAX) {
					DHD_ERROR(
						("%s : Unknown channel %d\n",
						 __FUNCTION__,
						 gscan_params->channel_bucket[i]
							 .chan_list[k]));
					return false;
				}
			}
				 ...
				 struct dhd_pno_gscan_params *gscan_params)
{
	...
	pfn_gscan_cfg_t =
		(wl_pfn_gscan_cfg_t *)MALLOCZ(dhd->osh, gscan_param_size);
	...
	if (gscan_params->mscan)
		pfn_gscan_cfg_t->buffer_threshold =
			gscan_params->buffer_threshold;
	else
		pfn_gscan_cfg_t->buffer_threshold = GSCAN_BATCH_NO_THR_SET;

	pfn_gscan_cfg_t->flags = (gscan_params->send_all_results_flag &
	...
	tot_nchan = pfn_gscan_cfg_t->channel_bucket[num_buckets_to_fw - 1]
			    .bucket_end_index +
		    1;
	DHD_PNO((
		"Total channel num %d total ch_buckets  %d ch_buckets_to_fw %d \n",
	...
	if ((err = _dhd_pno_gscan_cfg(dhd, pfn_gscan_cfg_t, gscan_param_size)) <
	    0) {
		DHD_ERROR((
			"%s : failed to set call pno_gscan_cfg (err %d) in firmware\n",
			__FUNCTION__, err));
		goto exit;
	}
	...
	if (gscan_params->nbssid_hotlist) {
		...
		GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
		list_for_each_entry_safe (
			iter, next, &gscan_params->hotlist_bssid_list, list) {
			char buffer_hotlist[64];
			GCC_DIAGNOSTIC_POP();
			memcpy(&ptr->macaddr, &iter->macaddr, ETHER_ADDR_LEN);
			BCM_REFERENCE(buffer_hotlist);
			DHD_PNO(("%s\n", bcm_ether_ntoa(&ptr->macaddr,
							buffer_hotlist)));
			ptr->flags = iter->flags;
			ptr++;
		}

		err = _dhd_pno_add_bssid(dhd, p_pfn_bssid,
					 gscan_params->nbssid_hotlist);
		if (err < 0) {
			DHD_ERROR((
				"%s : failed to call _dhd_pno_add_bssid(err :%d)\n",
				__FUNCTION__, err));
			goto exit;
		}
	}
	...
	*num_buckets_to_fw = 0;
	...
	if (!ch_bucket) {
		DHD_ERROR(("%s: failed to malloc memory of size %zd\n",
			   __FUNCTION__,
			   (*num_buckets) *
				   sizeof(wl_pfn_gscan_ch_bucket_cfg_t)));
	...
	for (i = 0; i < _params->params_gscan.nchannel_buckets && nchan; i++) {
		if (!gscan_buckets[i].band) {
			ch_cnt = MIN(gscan_buckets[i].num_channels,
				     (uint8)nchan);
			num_channels += ch_cnt;
		...
		ch_bucket[i].max_freq_multiple =
			gscan_buckets[i].bucket_max_multiple;
	...
	if (is_pno_legacy_running) {
		...
		if (nchan) {
			...
			max = gscan_buckets[0].bucket_freq_multiple;
			/* GSCAN buckets */
			for (i = 0; i < _params->params_gscan.nchannel_buckets;
			     i++) {
				ch_bucket[i].bucket_freq_multiple *=
					_params->params_gscan.scan_fr;
				ch_bucket[i].bucket_freq_multiple /=
			...
			_params->params_gscan.max_ch_bucket_freq =
				MAX(max, ch_bucket[legacy_bucket_idx]
						 .bucket_freq_multiple);
			ch_bucket[legacy_bucket_idx].flag =
				CH_BUCKET_REPORT_REGULAR;
			/* Now add channels to the legacy scan bucket */
			for (i = 0; i < _params1->params_legacy.nchan && nchan;
			     i++, nchan--) {
...
static int dhd_pno_stop_for_gscan(dhd_pub_t *dhd)
{
	...
	if (_pno_state->pno_params_arr[INDEX_OF_GSCAN_PARAMS]
		    .params_gscan.mscan) {
		/* retrieve the batching data from firmware into host */
		err = dhd_wait_batch_results_complete(dhd);
		if (err != BCME_OK)
	...
	if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
		...
		err = dhd_pno_set_legacy_pno(dhd, params_legacy->scan_fr,
					     params_legacy->pno_repeat,
					     params_legacy->pno_freq_expo_max,
...
int dhd_pno_initiate_gscan_request(dhd_pub_t *dhd, bool run, bool flush)
{
	...
	if (run) {
		...
		if (flush) {
			mutex_lock(&_pno_state->pno_mutex);
			dhd_pno_reset_cfg_gscan(dhd, params, _pno_state,
						GSCAN_FLUSH_ALL_CFG);
			mutex_unlock(&_pno_state->pno_mutex);
		}
...
int dhd_pno_enable_full_scan_result(dhd_pub_t *dhd, bool real_time_flag)
{
	...
	if (_pno_state->pno_mode & DHD_PNO_GSCAN_MODE) {
		if (old_flag != gscan_params->send_all_results_flag) {
			...
			if ((err = _dhd_pno_gscan_cfg(
				     dhd, &gscan_cfg,
				     sizeof(wl_pfn_gscan_cfg_t))) < 0) {
				DHD_ERROR((
					"%s : pno_gscan_cfg failed (err %d) in firmware\n",
					__FUNCTION__, err));
...
static int _dhd_pno_get_gscan_batch_from_fw(dhd_pub_t *dhd)
{
	...
	plbestnet_v1 =
		(wl_pfn_lscanresults_v1_t *)MALLOC(dhd->osh, PNO_BESTNET_LEN);
	if (!plbestnet_v1) {
		DHD_ERROR(("%s :Out of memory!! Cant malloc %d bytes\n",
			   __FUNCTION__, (int)PNO_BESTNET_LEN));
		err = BCME_NOMEM;
		goto exit;
	}
	...
	do {
		err = dhd_iovar(dhd, 0, "pfnlbest", NULL, 0,
				(char *)plbestnet_v1, PNO_BESTNET_LEN, FALSE);
		if (err < 0) {
			DHD_ERROR((
				"%s : Cannot get all the batch results, err :%d\n",
				__FUNCTION__, err));
			goto exit_mutex_unlock;
		}
		...
		if (plbestnet_v1->version == PFN_LBEST_SCAN_RESULT_VERSION_V1) {
			...
			if (fwcount > BESTN_MAX) {
				DHD_ERROR((
					"%s :fwcount %d is greater than BESTN_MAX %d \n",
			 ...
			 */
			for (i = 0, count = 0; i < fwcount;
			     i++, count++, plnetinfo++) {
				/* Unlikely to happen, but just in case the results from
				 * FW doesnt make sense..... Assume its part of one single scan
				 */
				if (num_scans_in_cur_iter >=
				    gscan_params->mscan) {
					num_scans_in_cur_iter = 0;
					count = fwcount;
					break;
				}
				if (TIME_DIFF_MS(timestamp,
						 plnetinfo->timestamp) >
				    timediff) {
					nAPs_per_scan[num_scans_in_cur_iter] =
						count;
					count = 0;
			...
			for (i = 0; i < num_scans_in_cur_iter; i++) {
				...
				if (!iter) {
					DHD_ERROR((
						"%s :Out of memory!! Cant malloc %d bytes\n",
						__FUNCTION__,
						gscan_params->mscan));
					err = BCME_NOMEM;
					goto exit_mutex_unlock;
				 ...
				 */
				if (TIME_DIFF_MS(ts, plnetinfo->timestamp) >
				    timediff) {
				...
				for (j = 0; j < nAPs_per_scan[i];
					...
					result->channel = wl_channel_to_frequency(
						wf_chspec_ctlchan(
							plnetinfo->pfnsubnet
								.channel),
						CHSPEC_BAND(plnetinfo->pfnsubnet
								    .channel));
					...
					result->rtt_sd =
						(uint64)plnetinfo->rtt1;
					result->ts =
						convert_fw_rel_time_to_systime(
							&tm_spec,
							plnetinfo->timestamp);
					ts = plnetinfo->timestamp;
					if (plnetinfo->pfnsubnet.SSID_len >
					    DOT11_MAX_SSID_LEN) {
						DHD_ERROR((
							"%s: Invalid SSID length %d\n",
							__FUNCTION__,
							plnetinfo->pfnsubnet
								.SSID_len));
					...
					(void)memcpy_s(
						...
						plnetinfo->pfnsubnet.SSID_len);
					result->ssid[plnetinfo->pfnsubnet
							     .SSID_len] = '\0';
					(void)memcpy_s(
						&result->macaddr,
						ETHER_ADDR_LEN,
						&plnetinfo->pfnsubnet.BSSID,
						ETHER_ADDR_LEN);

					DHD_PNO(("\tSSID : "));
					DHD_PNO(("\n"));
					DHD_PNO(("\tBSSID: " MACDBG "\n",
						 MAC2STRDBG(result->macaddr
								    .octet)));
					DHD_PNO((
						"\tchannel: %d, RSSI: %d, timestamp: %d ms\n",
		...
		} else if (plbestnet_v2->version ==
			   PFN_LBEST_SCAN_RESULT_VERSION_V2) {
			fwstatus = plbestnet_v2->status;
			fwcount = plbestnet_v2->count;
			plnetinfo_v2 = (wl_pfn_lnet_info_v2_t *)&plbestnet_v2
					       ->netinfo[0];
			...
			for (i = 0; i < num_scans_in_cur_iter; i++) {
				 ...
				 * maybe a continuation of previous sets' scan results
				 */
				if (TIME_DIFF_MS(ts, plnetinfo_v2->timestamp) >
				    timediff) {
					iter->scan_id = ++gscan_params->scan_id;
				} else {
					iter->scan_id = gscan_params->scan_id;
				...
				DHD_PNO((
					"scan_id %d tot_count %d ch_bucket %x\n",
				...
				for (j = 0; j < nAPs_per_scan[i];
					...
					result->channel = wl_channel_to_frequency(
						wf_chspec_ctlchan(
							plnetinfo_v2->pfnsubnet
								.channel),
						CHSPEC_BAND(
							plnetinfo_v2->pfnsubnet
								.channel));
					result->rssi =
						(int32)plnetinfo_v2->RSSI;
					/* Info not available & not expected */
					result->beacon_period = 0;
					result->capability = 0;
					result->rtt =
						(uint64)plnetinfo_v2->rtt0;
					result->rtt_sd =
						(uint64)plnetinfo_v2->rtt1;
					result->ts =
						convert_fw_rel_time_to_systime(
							&tm_spec,
							plnetinfo_v2->timestamp);
					ts = plnetinfo_v2->timestamp;
					if (plnetinfo_v2->pfnsubnet.SSID_len >
					    DOT11_MAX_SSID_LEN) {
						DHD_ERROR((
							"%s: Invalid SSID length %d\n",
							__FUNCTION__,
					...
					(void)memcpy_s(
						...
						plnetinfo_v2->pfnsubnet
							.SSID_len);
					result->ssid[plnetinfo_v2->pfnsubnet
							     .SSID_len] = '\0';
					(void)memcpy_s(
						&result->macaddr,
						ETHER_ADDR_LEN,
						&plnetinfo_v2->pfnsubnet.BSSID,
						ETHER_ADDR_LEN);
...
exit:
	...
	if (nAPs_per_scan) {
		MFREE(dhd->osh, nAPs_per_scan,
		      gscan_params->mscan * sizeof(uint8));
	}
	if (plbestnet_v1) {
			...
			void *info, uint32 *len)
{
	...
	case DHD_PNO_GET_NEW_EPNO_SSID_ELEM:
		...
		if (epno_cfg->num_epno_ssid >= MAX_EPNO_SSID_NUM) {
			DHD_ERROR((
				"Excessive number of ePNO SSIDs programmed %d\n",
				  ...
				  int reason)
{
	 ...
	 */
	STATIC_ASSERT(sizeof(wl_pfn_net_info_v1_t) ==
		      sizeof(wl_pfn_net_info_v2_t));
	...
	_params = &_pno_state->pno_params_arr[INDEX_OF_BATCH_PARAMS];
	if (buf && bufsize) {
		if (!list_empty(&_params->params_batch.get_batch
					 .expired_scan_results_list)) {
			...
			if (!list_empty(&_params->params_batch.get_batch
				...
				list_for_each_entry_safe (
					siter, snext,
					&_params->params_batch.get_batch
						 .scan_results_list,
					list) {
					GCC_DIAGNOSTIC_POP();
					list_move_tail(
	...
	pscan_results =
		(dhd_pno_scan_results_t *)MALLOC(dhd->osh, SCAN_RESULTS_SIZE);
	...
	if (_params->params_batch.get_batch.top_node_cnt < MAX_NODE_CNT) {
		...
		_removed_scan_cnt = _dhd_pno_clear_all_batch_results(
			dhd, &_params->params_batch.get_batch.scan_results_list,
			TRUE);
		_params->params_batch.get_batch.tot_scan_cnt -=
			_removed_scan_cnt;
		list_add(&pscan_results->list,
	...
	if (!plbestnet_v1) {
		err = BCME_NOMEM;
		DHD_ERROR(("%s: failed to allocate buffer for bestnet",
			   __FUNCTION__));
		goto exit;
	}
	...
	do {
		...
		if (plbestnet_v1->version == PFN_LBEST_SCAN_RESULT_VERSION_V1) {
			...
			for (i = 0; i < fwcount; i++) {
				pbestnet_entry =
					(dhd_pno_bestnet_entry_t *)MALLOC(
						dhd->osh, BESTNET_ENTRY_SIZE);
				if (pbestnet_entry == NULL) {
					err = BCME_NOMEM;
					DHD_ERROR((
						"failed to allocate dhd_pno_bestnet_entry\n"));
					goto exit;
				}
				...
				if (allocate_header) {
					...
					if (reason)
						pbestnetheader->reason =
							(ENABLE << reason);
					memset(pbestnetheader, 0,
					       BEST_HEADER_SIZE);
					/* initialize the head of linked list */
					INIT_LIST_HEAD(
						&(pbestnetheader->entry_list));
					/* link the pbestnet heaer into existed list */
					if (pscan_results->bestnetheader ==
					    NULL)
						/* In case of header */
				...
				pbestnet_entry->SSID_len =
					plnetinfo->pfnsubnet.SSID_len;
				memcpy(pbestnet_entry->SSID,
				       plnetinfo->pfnsubnet.SSID,
				       pbestnet_entry->SSID_len);
				memcpy(&pbestnet_entry->BSSID,
				       &plnetinfo->pfnsubnet.BSSID,
				       ETHER_ADDR_LEN);
				/* add the element into list */
				list_add_tail(&pbestnet_entry->list,
					      &pbestnetheader->entry_list);
				...
				DHD_PNO(("\tSSID : "));
				for (j = 0; j < plnetinfo->pfnsubnet.SSID_len;
				     j++)
					DHD_PNO(("%c",
						 plnetinfo->pfnsubnet.SSID[j]));
				DHD_PNO(("\n"));
				DHD_PNO(("\tBSSID: " MACDBG "\n",
					 MAC2STRDBG(plnetinfo->pfnsubnet.BSSID
							    .octet)));
				DHD_PNO((
					"\tchannel: %d, RSSI: %d, timestamp: %d ms\n",
					plnetinfo->pfnsubnet.channel,
			...
			for (i = 0; i < fwcount; i++) {
				...
				DHD_PNO(("\tSSID : "));
				for (j = 0;
				     j < plnetinfo_v2->pfnsubnet.SSID_len; j++)
					DHD_PNO(("%c", plnetinfo_v2->pfnsubnet.u
							       .SSID[j]));
				DHD_PNO(("\n"));
				DHD_PNO(("\tBSSID: " MACDBG "\n",
					 MAC2STRDBG(plnetinfo_v2->pfnsubnet
							    .BSSID.octet)));
				DHD_PNO((
					"\tchannel: %d, RSSI: %d, timestamp: %d ms\n",
					plnetinfo_v2->pfnsubnet.channel,
	...
	if (pscan_results->cnt_header == 0) {
		 ...
		 */
		DHD_PNO((
			"NO BATCH DATA from Firmware, Delete current SCAN RESULT LIST\n"));
	...
	convert_format:
		err = _dhd_pno_convert_format(dhd, &_params->params_batch, buf,
					      bufsize);
		if (err < 0) {
			DHD_ERROR((
				"failed to convert the data into upper layer format\n"));
			goto exit;
		}
	}
exit:
...
static void _dhd_pno_get_batch_handler(struct work_struct *work)
{
	...
	if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
		params_batch =
			&_pno_state->pno_params_arr[INDEX_OF_BATCH_PARAMS]
				 .params_batch;
...
int dhd_pno_stop_for_batch(dhd_pub_t *dhd)
{
	...
	if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
		DHD_ERROR(
			("%s : PNO BATCH MODE is not enabled\n", __FUNCTION__));
		goto exit;
	}
	_pno_state->pno_mode &= ~DHD_PNO_BATCH_MODE;
	if (_pno_state->pno_mode &
	    (DHD_PNO_LEGACY_MODE | DHD_PNO_HOTLIST_MODE)) {
		mode = _pno_state->pno_mode;
		err = dhd_pno_clean(dhd);
		if (err < 0) {
			DHD_ERROR((
		...
		if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
			...
			_params_legacy = &(
				_pno_state
					->pno_params_arr[INDEX_OF_LEGACY_PARAMS]
					.params_legacy);
			err = dhd_pno_set_legacy_pno(
				dhd, _params_legacy->scan_fr,
				_params_legacy->pno_repeat,
				_params_legacy->pno_freq_expo_max,
				_params_legacy->chan_list,
			...
			list_for_each_entry_safe (
				...
				list) {
				GCC_DIAGNOSTIC_POP();
				memcpy(&p_pfn_bssid[i].macaddr, &iter->macaddr,
				       ETHER_ADDR_LEN);
			    ...
			    struct dhd_pno_hotlist_params *hotlist_params)
{
	...
	if (hotlist_params->nchan)
		memcpy(_params->params_hotlist.chan_list,
		       hotlist_params->chan_list,
		       sizeof(_params->params_hotlist.chan_list));
	memset(_chan_list, 0, sizeof(_chan_list));

	rem_nchan =
		ARRAYSIZE(hotlist_params->chan_list) - hotlist_params->nchan;
	if (hotlist_params->band == WLC_BAND_2G ||
		...
		err = _dhd_pno_get_channels(
			dhd,
			&_params->params_hotlist
				 .chan_list[hotlist_params->nchan],
		...
		/* need to sort channel list */
		sort(_params->params_hotlist.chan_list,
		     _params->params_hotlist.nchan,
		     sizeof(_params->params_hotlist.chan_list[0]),
		     _dhd_pno_cmpfunc, NULL);
	}
#ifdef PNO_DEBUG
	{
	...
	for (i = 0; i < hotlist_params->nbssid; i++) {
		...
		memcpy(&_pno_bssid->macaddr, &p_pfn_bssid[i].macaddr,
		       ETHER_ADDR_LEN);
		_pno_bssid->flags = p_pfn_bssid[i].flags;
		list_add_tail(&_pno_bssid->list,
			      &_params->params_hotlist.bssid_list);
	}
...
int dhd_retreive_batch_scan_results(dhd_pub_t *dhd)
{
	...
	if (_params->params_gscan.get_batch_flag ==
		...
		_params->params_gscan.get_batch_flag =
			GSCAN_BATCH_RETRIEVAL_IN_PROGRESS;
		smp_wmb();
		schedule_work(&_pno_state->work);
	} else {
				    ...
				    uint32 len, int *size)
{
	...
	*size = 0;
	...
	if (bi_length !=
	    (dtoh32(gscan_result->buflen) - WL_GSCAN_RESULTS_FIXED_SIZE -
	...
	mem_needed =
		OFFSETOF(wifi_gscan_full_result_t, ie_data) + bi->ie_length;
	...
	struct dhd_pno_gscan_params *gscan_params, uint32 event)
{
	...
	if ((pfn_result->count == 0) ||
	    (pfn_result->count > EVENT_MAX_NETCNT_V3)) {
				  ...
				  uint32 event, int *size)
{
	...
	*size = 0;
	...
	if (event == WLC_E_PFN_NET_FOUND || event == WLC_E_PFN_NET_LOST) {
		wl_pfn_scanresults_v1_t *pfn_result =
			(wl_pfn_scanresults_v1_t *)data;
		wl_pfn_scanresults_v2_t *pfn_result_v2 =
			(wl_pfn_scanresults_v2_t *)data;
		wl_pfn_scanresults_v3_t *pfn_result_v3 =
			(wl_pfn_scanresults_v3_t *)data;
		...
		if (pfn_result->version == PFN_SCANRESULT_VERSION_V1) {
			...
			results = (dhd_epno_results_t *)MALLOC(dhd->osh,
							       mem_needed);
			if (!results) {
				DHD_ERROR((
			...
			for (i = 0; i < count; i++) {
				...
				results[i].channel = wf_channel2mhz(
					...
					(net->pfnsubnet.channel <=
						 ...
						 WF_CHAN_FACTOR_5_G));
				results[i].flags =
					(event == WLC_E_PFN_NET_FOUND) ?
						WL_PFN_SSID_EXT_FOUND :
						WL_PFN_SSID_EXT_LOST;
				results[i].ssid_len =
					min(net->pfnsubnet.SSID_len,
					    (uint8)DOT11_MAX_SSID_LEN);
				bssid = &results[i].bssid;
				(void)memcpy_s(bssid, ETHER_ADDR_LEN,
					       &net->pfnsubnet.BSSID,
				...
				(void)memcpy_s(ssid, DOT11_MAX_SSID_LEN,
					       ...
					       results[i].ssid_len);
				ssid[results[i].ssid_len] = '\0';
				DHD_PNO(("ssid - %s bssid " MACDBG
					 " ch %d rssi %d flags %d\n",
					 ssid, MAC2STRDBG(bssid->octet),
					 results[i].channel, results[i].rssi,
					 results[i].flags));
			}
		} else if (pfn_result_v2->version ==
			   PFN_SCANRESULT_VERSION_V2) {
			if ((pfn_result->count == 0) ||
			    (pfn_result->count > EVENT_MAX_NETCNT_V2)) {
				DHD_ERROR((
					"%s event %d: wrong pfn v2 results count %d\n",
					__FUNCTION__, event,
					pfn_result->count));
				return NULL;
			}
			...
			for (i = 0; i < count; i++) {
				...
				results[i].flags =
					(event == WLC_E_PFN_NET_FOUND) ?
						...
						WL_PFN_SSID_EXT_LOST;
				results[i].ssid_len =
					min(net_v2->pfnsubnet.SSID_len,
					    (uint8)DOT11_MAX_SSID_LEN);
				bssid = &results[i].bssid;
				(void)memcpy_s(bssid, ETHER_ADDR_LEN,
					       &net_v2->pfnsubnet.BSSID,
					       ETHER_ADDR_LEN);
				if (!net_v2->pfnsubnet.SSID_len) {
					dhd_pno_idx_to_ssid(
						gscan_params, &results[i],
						net_v2->pfnsubnet.u.index);
				} else {
					(void)memcpy_s(results[i].ssid,
						       DOT11_MAX_SSID_LEN,
						       net_v2->pfnsubnet.u.SSID,
						       results[i].ssid_len);
				}
				(void)memcpy_s(ssid, DOT11_MAX_SSID_LEN,
					       results[i].ssid,
		...
		} else if (pfn_result_v3->version ==
			   PFN_SCANRESULT_VERSION_V3) {
			results = dhd_pno_update_pfn_v3_results(
				dhd, pfn_result_v3, &mem_needed, gscan_params,
				event);
			if (results == NULL) {
				return results;
			}
			...
			DHD_ERROR((
				"%s event %d: Incorrect version %d , not supported\n",
...
static void *dhd_pno_update_hotlist_v3_results(
	dhd_pub_t *dhd, wl_pfn_scanresults_v3_t *pfn_result,
	...
	*buf_len = malloc_size;
	...
	for (i = 0; i < pfn_result->count; i++, pnetinfo++) {
		...
		if (pnetinfo->pfnsubnet.SSID_len > DOT11_MAX_SSID_LEN) {
			DHD_ERROR(
				("Invalid SSID length %d: trimming it to max\n",
				  ...
				  u32 *buf_len)
{
	...
	dhd_pno_status_info_t *_pno_state = PNO_GET_PNOSTATE(dhd);
	struct dhd_pno_gscan_params *gscan_params;
	wl_pfn_scanresults_v1_t *results_v1 =
		(wl_pfn_scanresults_v1_t *)event_data;
	wl_pfn_scanresults_v2_t *results_v2 =
		(wl_pfn_scanresults_v2_t *)event_data;
	wl_pfn_scanresults_v3_t *results_v3 =
		(wl_pfn_scanresults_v3_t *)event_data;
	wifi_gscan_result_t *hotlist_found_array;
	...
	*buf_len = 0;
	if (results_v1->version == PFN_SCANRESULTS_VERSION_V1) {
		...
		*buf_len = malloc_size;
		if (type == HOTLIST_FOUND) {
			gscan_hotlist_cache->next =
				gscan_params->gscan_hotlist_found;
			gscan_params->gscan_hotlist_found = gscan_hotlist_cache;
			DHD_PNO(("%s enter, FOUND results count %d\n",
		...
		for (i = 0; i < fwcount; i++, pnetinfo++) {
			hotlist_found_array = &gscan_hotlist_cache->results[i];
			memset(hotlist_found_array, 0,
			       sizeof(wifi_gscan_result_t));
			hotlist_found_array->channel = wf_channel2mhz(
				pnetinfo->pfnsubnet.channel,
				(pnetinfo->pfnsubnet.channel <=
						 CH_MAX_2G_CHANNEL ?
					 WF_CHAN_FACTOR_2_4_G :
					 WF_CHAN_FACTOR_5_G));
			...
			(void)memcpy_s(hotlist_found_array->ssid,
				       DOT11_MAX_SSID_LEN,
				       pnetinfo->pfnsubnet.SSID,
				       pnetinfo->pfnsubnet.SSID_len);
			hotlist_found_array->ssid[pnetinfo->pfnsubnet.SSID_len] =
				'\0';

			(void)memcpy_s(&hotlist_found_array->macaddr,
				       ETHER_ADDR_LEN,
				       &pnetinfo->pfnsubnet.BSSID,
				       ETHER_ADDR_LEN);
			DHD_PNO(("\t%s " MACDBG " rssi %d\n",
				 hotlist_found_array->ssid,
				 MAC2STRDBG(hotlist_found_array->macaddr.octet),
		...
		*buf_len = malloc_size;
		...
		gscan_hotlist_cache->tot_count = fwcount;
		gscan_hotlist_cache->tot_consumed = 0;
		gscan_hotlist_cache->scan_ch_bucket =
			results_v2->scan_ch_bucket;
		...
		for (i = 0; i < fwcount; i++, pnetinfo_v2++) {
			...
			if (pnetinfo_v2->pfnsubnet.SSID_len >
			    DOT11_MAX_SSID_LEN) {
				DHD_ERROR((
					"Invalid SSID length %d: trimming it to max\n",
			...
			(void)memcpy_s(hotlist_found_array->ssid,
				       ...
				       pnetinfo_v2->pfnsubnet.SSID_len);
			hotlist_found_array
				->ssid[pnetinfo_v2->pfnsubnet.SSID_len] = '\0';

			(void)memcpy_s(&hotlist_found_array->macaddr,
				       ETHER_ADDR_LEN,
				       &pnetinfo_v2->pfnsubnet.BSSID,
				       ETHER_ADDR_LEN);
			DHD_PNO(("\t%s " MACDBG " rssi %d\n",
				 hotlist_found_array->ssid,
				 MAC2STRDBG(hotlist_found_array->macaddr.octet),
		...
		gscan_hotlist_cache = (gscan_results_cache_t *)
			  ...
			  void *event_data)
{
...
#ifndef GSCAN_SUPPORT
	{
		...
		params_batch =
			&_pno_state->pno_params_arr[INDEX_OF_BATCH_PARAMS]
				 .params_batch;
		if (!COMPLETION_WAIT_QUEUE_ACTIVE(
			    &_pno_state->get_batch_done.wait)) {
